...
-- فرض بر این است که Linked Server با نام 'UMD' قبلاً تنظیم شده و به دیتابیس 'UMD' روی سرور 192.168.1.24 اشاره می‌کند.
-- همان‌طور که در کوئری تست شما ([UMD].[UMD].[dbo].[users]) نشان می‌دهد، مسیر چهار بخشی برای دسترسی به جدول‌ها استفاده می‌شود.
...
-- FOREIGN KEYها حذف شده‌اند (چون به VIEWها اشاره می‌کردند و مجاز نبود)، پس یکپارچگی داده‌ها را در اپ یا تریگرها مدیریت کنید.
-- ستون PasswordHash از VIEW Users حذف شده.
...
-- ابتدا جدول‌های فیزیکی اسکیمای اصلی که VIEW نیستند (بدون تغییرات، بر اساس db-schema.sql.txt، اما بدون FOREIGN KEY به VIEWها)
...
-- حالا VIEWها با نام‌های اصلی جدول‌ها، که از دیتابیس UMD داده می‌کشند (با مسیر چهار بخشی [UMD].[UMD].[dbo].[table])
...
-- مسیر جدول‌ها به چهار بخشی تغییر داده شده ([UMD].[UMD].[dbo].[table]).
...
-- چون FOREIGN KEYها حذف شده‌اند، یکپارچگی داده‌ها را در اپ یا با چک‌های اضافی در تریگرها مدیریت کنید (مثلاً چک کنید UserId وجود داشته باشد قبل از INSERT به جدول‌های فیزیکی).
