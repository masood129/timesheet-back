-- =============================================
-- کوئری 2: ساخت جدول‌های مورد نیاز برنامه
-- =============================================
-- این اسکریپت برای ایجاد جدول‌های جدید برنامه timesheet در دیتابیس UMD است
-- این جدول‌ها در کنار جدول‌های موجود (users, groups, projects, groupManagers) قرار می‌گیرند
-- =============================================
...
IF OBJECT_ID('dbo.MonthlyGymCosts', 'U') IS NOT NULL DROP TABLE dbo.MonthlyGymCosts;
IF OBJECT_ID('dbo.DailyPersonalCarCosts', 'U') IS NOT NULL DROP TABLE dbo.DailyPersonalCarCosts;
IF OBJECT_ID('dbo.DailyProjectTasks', 'U') IS NOT NULL DROP TABLE dbo.DailyProjectTasks;
IF OBJECT_ID('dbo.DailyDetails', 'U') IS NOT NULL DROP TABLE dbo.DailyDetails;
IF OBJECT_ID('dbo.UserContractHours', 'U') IS NOT NULL DROP TABLE dbo.UserContractHours;
IF OBJECT_ID('dbo.UserProjectAccess', 'U') IS NOT NULL DROP TABLE dbo.UserProjectAccess;
IF OBJECT_ID('dbo.MonthPeriodSettings', 'U') IS NOT NULL DROP TABLE dbo.MonthPeriodSettings;
IF OBJECT_ID('dbo.Admins', 'U') IS NOT NULL DROP TABLE dbo.Admins;
GO

-- حذف functionها و stored procedureها
IF OBJECT_ID('dbo.fn_GetMonthLength', 'FN') IS NOT NULL DROP FUNCTION dbo.fn_GetMonthLength;
IF OBJECT_ID('dbo.fn_IsMonthEditable', 'FN') IS NOT NULL DROP FUNCTION dbo.fn_IsMonthEditable;
IF OBJECT_ID('dbo.sp_GetMonthPeriod', 'P') IS NOT NULL DROP PROCEDURE dbo.sp_GetMonthPeriod;
IF OBJECT_ID('dbo.sp_GetYearMonthPeriods', 'P') IS NOT NULL DROP PROCEDURE dbo.sp_GetYearMonthPeriods;
GO
...
-- پیام موفقیت
-- =============================================
PRINT N'✓ تمام جدول‌های مورد نیاز برنامه با موفقیت در دیتابیس UMD ایجاد شدند:';
PRINT N'  - Admins (مدیران سیستم)';
PRINT N'  - UserProjectAccess (دسترسی کاربران به پروژه‌ها)';
PRINT N'  - UserContractHours (قراردادهای ساعت کاری)';
PRINT N'  - DailyDetails (جزئیات روزانه)';
PRINT N'  - DailyProjectTasks (وظایف پروژه روزانه)';
PRINT N'  - DailyPersonalCarCosts (هزینه‌های ماشین شخصی)';
PRINT N'  - MonthlyGymCosts (هزینه‌های ورزش ماهیانه)';
PRINT N'  - MonthlyReports (گزارش‌های ماهیانه)';
PRINT N'  - MonthPeriodSettings (تنظیمات بازه ماه‌ها)';
PRINT N'✓ Functionها و Stored Procedureها نیز ایجاد شدند.';
PRINT N'✓ مرحله بعدی: اجرای کوئری 03-UMD-test-data.sql برای ساخت داده‌های تستی';
GO
